...
def makesurf(start, stop, field, datadir, save_plots=True, data=None):
    ...
    trim_mean_frac = 0.2  # frac of data points to chop from each end of frac Stokes value dist
    ...
    xnew = np.arange(np.min(x), np.max(
        x)+grid_point_sep_deg, grid_point_sep_deg)
    ynew = np.arange(np.min(y), np.max(
        y)+grid_point_sep_deg, grid_point_sep_deg)
    ...
    for row_idx, row in enumerate(tqdm(pair_dist)):
        ...
        if len(q_of_points_in_aperture) >= min_data_points_in_aperture:
            est_q_leak_of_points_in_aperture = trim_mean(
                q_of_points_in_aperture)
        ...
        if len(u_of_points_in_aperture) >= min_data_points_in_aperture:
            est_u_leak_of_points_in_aperture = trim_mean(
                u_of_points_in_aperture)
        ...
        if len(p_of_points_in_aperture) >= min_data_points_in_aperture:
            est_p_leak_of_points_in_aperture = trim_mean(
                p_of_points_in_aperture)
